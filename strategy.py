...
def calc_vwap_session(highs, lows, closes, volumes, timestamps=None):
    ...
    # Heures de reset UTC (heure, minute)
    SESSION_RESETS = [
        (0,  0),   # Asie    — minuit UTC
        (7,  0),   # Londres — 07h00 UTC
        (13, 30),  # New York — 13h30 UTC
    ]

    def get_session_id(dt_utc):
        """Retourne un identifiant unique de session."""
        h = dt_utc.hour; m = dt_utc.minute
        total_min = h * 60 + m
        # Session Asie     : 00h00 → 06h59
        # Session Londres  : 07h00 → 13h29
        # Session New York : 13h30 → 23h59
...
def calc_signal(candles_5m, candles_1m,
    ...
    # Extraire timestamps pour reset VWAP à minuit UTC
