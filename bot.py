...
def open_position(signal, risk_pct, *, best_bid=None, best_ask=None):
    ...
    reste sous le plafond global. RR, SL, TP1 et TP2 restent ceux fournis par
    la stratégie.
    """
    if len(state.positions) >= MAX_POSITIONS:
        return False, f"{len(state.positions)}/{MAX_POSITIONS} positions déjà ouvertes"
    ...
    if not pos["tp1_hit"]:
        ...
        if sl_hit:
            ...
            log.info(f"❌ SL [{pos['setup']}] {pos['trade_id']} ${ep:.2f}→${exit_p:.2f} | "
                     ...
                     f"Série:{state.consec_losses} | Slots:{len(state.positions)}/{MAX_POSITIONS}")
        ...
        if sl2_hit:
            ...
            _remove_position(pos)
            save_state()
            log.info(f"✅ TP1+BE [{pos['setup']}] {pos['trade_id']} | Lot2 BE ${exit_p:.2f} | "
        ...
        if tp2_hit:
            ...
            log.info(f"🎯 TP2 [{pos['setup']}] {pos['trade_id']} ${pos['tp2']:.2f} | "
                     ...
                     f"Capital:${state.paper_balance:.2f} | Slots:{len(state.positions)}/{MAX_POSITIONS}")
                ...
                if signal.get("signal"):
                    ...
                    for idx, candidate in enumerate(signals):
                        ...
                        if candidate.get("score", 0) < min_score_eff:
                            candidate["reason"] = (
                                f"Score {candidate['score']:.1f}<{min_score_eff:.1f}"
                                if PAPER_MODE
                                else f"DD N{state.dd_level} score {candidate['score']:.1f}<{min_score_eff:.1f}"
                            )
                            ...
                            if not opened:
                                candidate["reason"] = (
                                    f"{candidate.get('reason','Signal')} | NON OUVERT: {open_reason}"
                                )
